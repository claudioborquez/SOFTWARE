...
@login_required
def gestion_group(request):
    profile = Profile.objects.get(user_id=request.user.id)
    if profile.group_id != 1:
        messages.add_message(request, messages.INFO, 'Intenta ingresar a una area para la que no tiene permisos')
        return redirect('check_group_main')
    template_name = 'administrator/gestion_group.html'
...
def gestion_user(request):
    ...
    if profiles.group_id != 1 and profiles.group_id != 2:
        ...
        return redirect('check_group_main')
    groups = Group.objects.all().exclude(pk=0).order_by('id')
    template_name = 'administrator/gestion_user.html'
    return render(request,template_name,{'groups':groups,'profiles':profiles})
...
def new_group(request):
    ...
    if request.method == 'POST':
        ...
        return redirect('gestion_group')  # Reemplaza 'nombre_de_la_vista' con el nombre de la vista a la que deseas redirigir después de crear el grupo
...
def edit_group(request, group_id):
    group = get_object_or_404(Group, id=group_id)
    if request.method == 'POST':
        group.name = request.POST.get('nombre')
        group.save()
        return redirect('group_list')
    template_name = 'administrator/group_list.html'
    context = {
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.profile.first_session == 'Si':
                return redirect('pre_check_profile')
            else:
